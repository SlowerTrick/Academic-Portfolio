...
/**
 * @brief Recebe um vetor de conteúdos para o TAD Bitmap (Função nova adicionada para o Trabalho de ED 2)
 * 
 *
 * @param bm O mapa de bits.
 */
void bitmapSetContents(bitmap* bm, unsigned int tam, unsigned char* dados){
	if(bm){
		free(bm->contents);//Libera o espaco previamente alocado
	}
